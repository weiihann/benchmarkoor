...
- `run-capture` pins each worker container to the configured CPU and records `samples.jsonl`, `container.log`, and `exit.json` per session.
...
- **Smoke capture.** Run `taskset -c 0 bin/benchmarkoor run --config <home>/config/smoke-compute.yaml 2>&1 | tee results/verify/<run-id>/run.log`. The log ends with `Compute campaign artifacts retained run_dir=<home>/runs/compute-<uuid>`.
...
- Timings are only measurements when nothing else runs on the timed CPU and its SMT sibling. A verification capture proves correctness, not cost.
