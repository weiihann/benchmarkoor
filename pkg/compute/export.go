...
func writeGasfitRuntimes(path string, workload *Workload, results []Result) error {
	...
	header := []string{
		...
		"system_gas", "gas_before_settlement", "reason",
	}
	...
	for _, result := range results {
		...
		row := []string{
			"newl1", result.CaseID, durationMilliseconds(result.ExecutionDurationNS), result.SessionID,
			...
			uintString(result.ChargedGas), uintString(result.SystemGas), uintString(result.GasBeforeSettlement),
			resultReason(result),
		}
		for _, name := range parameterNames {
