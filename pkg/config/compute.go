...
const (
	...
	// maxNativeComputeTxGasCap is NewL1's native transaction gas limit.
	maxNativeComputeTxGasCap uint64 = 1 << 24
)

// ComputeConfig configures a Prague native-worker compute campaign.
...
// ValidateCompute checks only the native compute-campaign configuration. It is
// intentionally separate from Validate because compute campaigns do not use
// Ethereum clients, fixtures, or rollback strategies.
...
// Validate checks the native compute-campaign configuration.
...
func (c *ComputeGeneratorConfig) validate() error {
	...
	if c.TxGasCap > maxNativeComputeTxGasCap {
		return fmt.Errorf(
			"compute.generator.tx_gas_cap %d exceeds the native maximum %d",
			c.TxGasCap, maxNativeComputeTxGasCap,
		)
	}
	...
	for _, family := range c.Families {
		if !validComputeFamilies[family] {
			return fmt.Errorf("compute.generator.families: unsupported Prague compute family %q", family)
		}
...
func ValidateComputeQualificationPolicy(path string) error {
	...
	if !ok {
		return fmt.Errorf("compute.analyzer.config: campaign analysis requires a clients list containing %q", "newl1")
	}
	hasNewL1Client := false
	for _, client := range clients {
		if clientName, ok := client.(string); ok && clientName == "newl1" {
			hasNewL1Client = true
			break
		}
	}
