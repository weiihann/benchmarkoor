...
const (
	...
	senderKey    = "0x45a915e4d060149eb43659682e8a2380bc25f50c1bf0b2b0f0e0f1e0d0c0b0a0"
...
func baseRequest() *Request {
	return &Request{
		SchemaVersion: SchemaVersion,
		WorkloadPath:  "workloads/prague-compute.json",
...
func TestWorkloadValidateRejectsVersionAndForkMismatches(t *testing.T) {
	for name, mutate := range map[string]func(*Workload){
		...
		"schema_version future": func(w *Workload) { w.SchemaVersion = 2 },
		"fork osaka":            func(w *Workload) { w.Fork = "Osaka" },
...
func TestWorkloadValidateRejectsMalformedHex(t *testing.T) {
	for name, mutate := range map[string]func(*Workload){
		...
		"secret key short":         func(w *Workload) { w.Cases[0].Transactions[0].SecretKey = "0x1234" },
...
func TestRequestValidateRejectsVocabularyMismatches(t *testing.T) {
	for name, mutate := range map[string]func(*Request){
		"schema_version future": func(r *Request) { r.SchemaVersion = 2 },
		"unknown mode":          func(r *Request) { r.Mode = "instrumented" },
		"unsupported chain":     func(r *Request) { r.Chain = "mainnet" },
...
func TestResultValidateRejectsContradictoryUnsupportedResults(t *testing.T) {
	...
	for name, mutate := range map[string]func(*Result){
		...
		"carries system gas":   func(r *Result) { r.SystemGas = u64(21000) },
	} {
		t.Run(name, func(t *testing.T) {
			res := base()
			mutate(res)
...
func TestResultValidateRejectsUnknownVocabulary(t *testing.T) {
	for name, mutate := range map[string]func(*Result){
		"schema_version future": func(r *Result) { r.SchemaVersion = 2 },
...
func TestJSONFieldNamesMatchContract(t *testing.T) {
	t.Run("workload", func(t *testing.T) {
		...
		for _, key := range []string{"sender", "secret_key", "to", "data", "value", "gas_limit"} {
	...
	t.Run("request", func(t *testing.T) {
		...
		for _, key := range []string{"schema_version", "workload_path", "session_id", "mode", "chain", "samples"} {
			assert.Contains(t, top, key)
		}
	...
	t.Run("result", func(t *testing.T) {
		...
		for _, key := range []string{
			...
			"system_gas", "gas_before_settlement",
		} {
			assert.Contains(t, top, key)
		}
		// error is optional and absent when execution succeeded.
		assert.NotContains(t, top, "error")
		// Unavailable quantities are explicit nulls, not omitted or zero.
		assert.Nil(t, top["gas_before_settlement"])
	})
}
...
func TestDecodeRejectsInvalidNumbersAndShapes(t *testing.T) {
	t.Run("negative repetition", func(t *testing.T) {
		...
		err := json.Unmarshal([]byte(`{
			"schema_version": 1, "workload_path": "w.json", "session_id": "s",
			"mode": "diagnostic", "chain": "devnet",
	...
	t.Run("fractional duration", func(t *testing.T) {
		var res Result
		err := json.Unmarshal([]byte(`{
			"schema_version": 1, "session_id": "s", "sample_id": "a", "case_id": "b",
