...
func buildComputeManifest(
	...
	workloads := map[string]any{
		...
		"native_fixed_limits": nativeFixedLimits(cfg),
	}
	gasSchedule := map[string]any{
		"fork":                         SupportedFork,
		"native_transaction_gas_limit": maxNativeTransactionGas,
		"active_schedule_identity": map[string]string{
			"status": "unavailable",
			"reason": "native worker protocol does not expose a gas-schedule identity",
		},
	}
	return computeManifest{
		...
		Policy: map[string]any{
			"baseline":                 "fresh native process and worker-restored baseline for every requested sample",
...
const (
	...
	maxNativeTransactionGas  = uint64(1 << 24)
	defaultGeneratorTxGasCap = uint64(15_000_000)
)

func nativeFixedLimits(cfg *config.ComputeConfig) map[string]any {
	...
	return map[string]any{
		...
		"native_tx_gas_cap":    maxNativeTransactionGas,
	}
}
...
func collectSourceFacts(ctx context.Context, paths map[string]string) map[string]sourceFacts {
	names := []string{"benchmarkoor", "newl1", "execution_specs", "evm_gasfit"}
