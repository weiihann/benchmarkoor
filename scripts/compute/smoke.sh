#!/usr/bin/env bash
# Run a finite, real Prague compute pipeline control campaign.
#
...
# fixed target-count points. It detects lifetime regressions in the native
# worker's prepared-case state without substituting synthetic timings.
set -euo pipefail

repo_root="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)"
newl1_root="${repo_root}/../bnbchain-newL1"
...
for required_dir in "${newl1_root}" "${execution_specs_root}" "${gasfit_root}"; do
...
docker build -f "${repo_root}/Dockerfile.compute-worker" -t benchmarkoor-compute-worker:smoke "${newl1_root}"
...
if not performance or not all(
    ...
    raise SystemExit("performance samples must contain correct, measured native executions")
