...
// Build generates and validates a Prague workload package, or validates and
...
func requestLedgerRecord(request Request, sample RequestSample) map[string]any {
	return map[string]any{"schema_version": SchemaVersion, "session_id": request.SessionID, "mode": request.Mode, "chain": request.Chain, "sample_id": sample.SampleID, "case_id": sample.CaseID, "repetition": sample.Repetition, "phase": sample.Phase}
}
